...
add(){ local p="$1"; [[ -z "$p" ]] && return 0; blacklisted "$p" && { log "Skip blacklisted package: $p"; return 0; }; grep -Fxq "$p" "$OUT/.packages" 2>/dev/null || echo "$p" >> "$OUT/.packages"; }
...
copy_specs(){
  ...
  while IFS= read -r spec; do
    ...
    mkdir -p "$OUT/$pkg"; cp "$spec" "$OUT/$pkg/$pkg.spec"; add "$pkg"; ((n++))
...
discover(){
  ...
  declare -A seen=()
  log 'GitHub lookup'; : > .repos.tsv
  for term in "${TERMS[@]}"; do
    tmp=$(mktemp)
    curl -fsSL --retry 3 --retry-delay 2 \
      -H 'Accept: application/vnd.github+json' -H 'User-Agent: sonicde-build-queue' \
      "https://api.github.com/search/repositories?q=${term}%20in:name,description+org:${ORG}&per_page=100" -o "$tmp" || die "GitHub lookup failed: $term"

    while IFS= read -r full; do
      repo=${full##*/}; key=$(norm "$repo")
      [[ -z "$repo" || -z "$key" || -n "${seen[$key]:-}" ]] && continue
      seen[$key]=1
      blacklisted "$repo" && { log "Skip blacklisted GitHub repo: $repo"; continue; }
      printf '%s\t%s\n' "$repo" "$full" >> .repos.tsv; ((n++))
  ...
  sed 's#\t#\thttps://github.com/#' .repos.tsv > "$OUT/.repo-map.tsv"
...
fetch(){
  ...
  for b in "${BRANCHES[@]}"; do
    rm -rf "$OUT/$repo" "$tmp/repo.tar.gz"; mkdir -p "$OUT/$repo"
    archive="https://codeload.github.com/$full/tar.gz/$b"
    if curl -fsSL "$archive" -o "$tmp/repo.tar.gz" 2>"$OUT/$repo/.curl-$b.err" && \
       tar -xzf "$tmp/repo.tar.gz" -C "$OUT/$repo" --strip-components=1 2>"$OUT/$repo/.tar-$b.err" && \
       [[ -f "$OUT/$repo/$repo.spec" ]]; then
      log "Fetched package repo: $repo ($b)"; rm -rf "$tmp"; add "$repo"; return 0
    fi
  done
  rm -rf "$tmp" "$OUT/$repo"; die "could not fetch package repo with root spec: $repo"
}
...
queue(){
  ...
  log "Final package declarations: $(wc -l < "$OUT/.packages") packages"; sed 's/^/  /' "$OUT/.packages" | tee -a "$LOG" >&2
  git -C "$OUT" init -q; git -C "$OUT" add .; git -C "$OUT" -c user.name=builder -c user.email=builder@example.invalid commit --allow-empty -qm specs
  url="file://$PWD/$OUT"; ref=$(git -C "$OUT" rev-parse HEAD)
